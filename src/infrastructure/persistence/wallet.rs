    ...
    L: LedgerService + Send + Sync,
{
    async fn create_inter_crypto_transfer(
        ...
        // Resolve ledger accounts (source of truth)
        ...
        // Optional pre-check (UX). Not authoritative.
        let available_minor = self.ledger.peek_balance_minor(from_available).await?;
        ...
        let route =
            TransferRoute::crypto_transfer(region.clone(), from_user_id, destination).map_err(AppError::from)?;
        ...
        match self.ledger.hold_user_funds(hold_cmd).await {
            Ok(_) => Ok(InternalTransferResult::Success { intent }),
            Err(AppError::InsufficientFunds { available_minor, required_minor }) => {
                Ok(InternalTransferResult::RejectedInsufficientFunds { available_minor, required_minor })
            }
            Err(e) => Err(e),
        }
